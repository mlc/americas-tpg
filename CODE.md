...
**Pin placement + color in My Maps.** My Maps positions and colors pins from
the **`<StyleMap>` id**, not from the bundled image. Each marker's StyleMap id
follows My Maps' own scheme, `icon-1899-<RRGGBB>-nodesc` (catalog pin 1899
tinted by the hex color). Only that id scheme makes My Maps tip-anchor the pin
on the point — any other id is re-centered, so the pin drifts off the location
(badly when zoomed out, converging as you zoom in). Consequently **only the
color survives in My Maps** (target black; gold/silver/bronze for 1st/2nd/3rd;
red for last; gray otherwise) — the star/circle symbols are **not** shown there.

The color-baked PNGs under `images/` (from `assets/pins/`, regenerated by
`scripts/render-pins.sh`, requires `rsvg-convert`) are still bundled as each
style's `<Icon><href>` so the KMZ renders in *other* KML viewers (Google Earth,
geojson.io); My Maps ignores them. Re-run `render-pins.sh` after changing the
marker palette in `src/simplestyle.ts`.
