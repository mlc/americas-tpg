...
| `yarn build-kml` | Export every ended round to KMZ (default `rounds.kmz`) via `src/kml.ts`: one `<Folder>` per round, one `<Placemark>` per point, color-baked pin PNGs bundled under `images/`. More than 10 rounds split into `rounds-NNN-MMM.kmz` files of 10 rounds each (My Maps 10-layer cap). Takes `--rounds-dir <dir>` and `-o/--output <file>`. |
