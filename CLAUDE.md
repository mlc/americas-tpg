...
- **`language.ts`** — four hand-curated lookup tables for the 53 GADM countries reachable by the sampler: `GID0_TO_ISO639_1` (country → main language code), `GID0_TO_LOCAL_NAME` (country → name in its main language; e.g., `BRA → 'Brasil'`, `HTI → 'Ayiti'`), `ROUND_LABEL` (language code → translation of "Round"; `es → 'Ronda'`, `pt → 'Rodada'`, `fr → 'Manche'`, `nl → 'Ronde'`, `ht → 'Tou'`), and `RULES_LABEL` (language code → translation of "Rules"; `es → 'Reglas'`, `pt → 'Regras'`, `fr → 'Règles'`, `nl → 'Regels'`, `ht → 'Règ'`). Used at create-round time to localize the target country name, the Discord "Round" header, and the bilingual link text on the Rules link line.
...
The four tables in `language.ts` are coupled by domain rules: `GID0_TO_ISO639_1` and `GID0_TO_LOCAL_NAME` must cover the same set of GIDs, and every language used as a value must appear as a key in **both** `ROUND_LABEL` **and** `RULES_LABEL`. `language.test.ts` asserts all three invariants. **Adding a country means updating both GID-keyed maps; adding a language means also adding to `ROUND_LABEL` and `RULES_LABEL`** — otherwise the test fails (and silent partial localization would otherwise sneak through). The Haitian Creole label `Tou` was a deliberate pick over `Wonn` / `Manch`; leave it unless told otherwise. Localization applies only to the target — submission locations stay in GADM English. The selected language is persisted as `roundInfo.language` on the round file (not on the target's properties), and `formatTargetDiscord` reads it from there.

`formatTargetDiscord` produces a two-line message: a `# {Round-word} N, location, [coords](google-maps-url)` header followed by `[Rules / {translated}](RULES_URL)` on its own line. The Rules link text is **bilingual** for non-English rounds (English first, then translation, separated by ` / `) and plain `Rules` for English / unknown / missing language. `RULES_URL` (in `round-domain.ts`) points at `https://github.com/mlc/americas-tpg/blob/main/RULES.md` — the public-facing rules document; don't repoint it at a local path or fork URL. The result is consumed via `CreateRoundResult.targetLine` (name kept for compatibility — it's now multi-line) and printed by the create-round CLI.
