...
| `yarn build-kml` | Export every ended round to a single KMZ archive (default `rounds.kmz`) via `src/kml.ts`: one `<Folder>` per round, one `<Placemark>` per point, color-baked pin PNGs bundled under `images/`. Takes `--rounds-dir <dir>` and `-o/--output <file>`. |
