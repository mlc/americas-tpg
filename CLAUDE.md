...
| `yarn build-kml` | Export every ended round to KMZ (default `rounds.kmz`) via `src/kml.ts`: one `<Folder>` per round, one `<Placemark>` per point, color-baked pin PNGs bundled under `images/`. More than 10 rounds split into `rounds-NNN-MMM.kmz` files of 10 rounds each (My Maps 10-layer cap). `--only-players <file>` (one player name per line) restricts the export to those players; targets always included. Takes `--rounds-dir <dir>` and `-o/--output <file>`. |
...
- **`kml.ts`** (`yarn build-kml`) — exports ended rounds to one or more **KMZ** archives, mirroring `leaderboard.ts`'s shape: pure `buildRoundsKmlDocument` (the `doc.kml` XML) + pure `buildRoundsKmz` (zips doc + pins via `fflate`) + thin `generateKmz` + `main`. It is a **pure transcriber** of the simplestyle markers already on disk — `markerOf` reads `marker-symbol`/`marker-color` off each feature; it never recomputes eliminations. `generateKmz` filters to ended rounds (skips in-progress, like `generateLeaderboard`) and throws if none; the builders are ended-agnostic and throw only on empty input. **My Maps 10-layer cap:** each round is one `<Folder>` = one My Maps layer, and My Maps imports at most 10 layers, so `generateKmz` chunks the ended rounds into groups of `MAX_ROUNDS_PER_KMZ` (10) via the pure `chunkRounds` and returns one `KmzFile` per chunk (with its first/last round numbers). `main` writes one chunk verbatim to `-o`, but for a multi-chunk split names each file by round-number range via the pure `partOutputPath` (`rounds-001-010.kmz`, 3-digit padded like `roundPath`). The pure builders are chunk-agnostic — they still take an arbitrary `RoundFile[]`. **`--only-players <file>` filter:** when the optional `GenerateKmzDeps.onlyPlayers` set is present, `generateKmz` maps each ended round through the pure `filterRoundToPlayers` (keeps `targetOf` always + submissions whose `normalizePlayerName` is in the set) *before* pins/chunking, so pins and folder contents reflect the filter; `totalRounds` is unchanged (rounds aren't dropped, only non-matching submissions). `main` reads the file and builds the set with the pure `parseOnlyPlayers` (one normalized name per line, blanks dropped). Matching is the repo-standard case-sensitive normalized comparison. Target placemarks are always included regardless of the filter. Each round → one `<Folder>` (a KML layer); each feature → one `<Placemark>` named `properties.player` (`Target` for the target), carrying `properties.location` and `properties.distance` (km, `toFixed(3)`) as KML `<ExtendedData><Data>` entries so My Maps shows them on pin click — emitted only when present, so the target (distance `null`) gets a location `<Data>` but no distance one. **Load-bearing — the `icon-1899-<RRGGBB>-nodesc` StyleMap id is what positions and colours pins in My Maps.** My Maps draws its *own* catalog pin (number 1899) tinted by the RRGGBB in the StyleMap id, and only tip-anchors the icon when the id matches that `icon-<n>-<RRGGBB>[-nodesc]` scheme; any other id (a bare/foreign style id) is re-centred, so the pin floats off the true point — a fixed screen-pixel offset that looks far off zoomed out and converges as you zoom in. So per distinct pin colour, `buildRoundsKmlDocument` emits two `<Style>`s (`<styleId>-normal` labelScale 0, `<styleId>-highlight` labelScale 1) plus a `<StyleMap id="<styleId>">` where `styleId = myMapsStyleId(pinId)` (uppercase hex), and placemarks reference `#<styleId>`. Each IconStyle still carries `<Icon><href>images/<pinId>.png</href>`, `scale 1`, bottom-center `<hotSpot x="32" xunits="pixels" y="64" yunits="insetPixels"/>`; the StyleMap shape, the `x,xunits,y,yunits` hotSpot order, and the trailing `,0` altitude on `<coordinates>` all mirror My Maps' own export. **My Maps ignores the bundled PNG** (uses its catalog pin), so in My Maps only the *colour* distinguishes markers — the star/circle symbols are lost there; the PNGs exist so the KMZ renders in other KML viewers (Earth, geojson.io). This was established empirically (My Maps ignores `<IconStyle><color>` *and* custom-icon hrefs; an earlier `aabbggrr`-tint and a baked-PNG-only approach both failed) — don't revert the id scheme to a plain Style id. Pins live in `assets/pins/` (committed), regenerated by `scripts/render-pins.sh` (SVG → `rsvg-convert`); the filename is the pin-asset id `s_<symbol>_<rrggbb>` from `styleIdFor` (distinct from the My Maps `styleId`). `pinIdOf` clamps any unknown `(symbol,color)` to a known pin (star→target pin, else gray circle) so the KMZ never references a missing image; if the palette in `simplestyle.ts` changes, re-run the script and update `KNOWN_PIN_IDS`. Other gotcha: `marker-*` are **not** in `TargetProperties`/`SubmissionProperties`, so `markerOf` reads them via a widened `Record` lookup — don't widen the domain types to "fix" this. Built with `xmlbuilder2` (auto-escapes names/emoji). Output `rounds.kmz` is gitignored; this replaced the old GDAL `ogrmerge.py` script.
